...
const ChatAssistant = () => {
    ...
    const handleToggle = async () => {
        if (isOpen) {
            if (messages.length > 1) {
                try {
                    await axios.post('${import.meta.env.VITE_API_URL}/api/history/save', { messages });
    ...
    const handleSend = async () => {
        ...
        try {
            const response = await axios.post('${import.meta.env.VITE_API_URL}/api/chat', { prompt: input });
